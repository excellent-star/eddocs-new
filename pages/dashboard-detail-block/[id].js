...
const LISTE_ONE_BLOCK=gql`
    query LISTE_ONE_BLOCK($id:ID!) { 
      bloc(id:$id)
      {
        id
        libelle
...
export default function DashboardDetailBlock ({query}) {

    const {data,error,loading}=useQuery(LISTE_ONE_BLOCK,{
        variables:{
            id:query.id
        }
    });
    ...
    const details=data?.bloc.recoltes
  return (
    <>
      <PageWrapper
        headerConfig={{
                        ...
                        <div className="job-details-content pt-8 pl-sm-9 pl-6 pr-sm-9 pr-6 pb-10 border-bottom border-width-1 border-default-color light-mode-texts">
                            ...
                            <div className="row">
                                <div className="col-lg-12">
                                    <>Liste des recoltes de  {bloc.libelle} </>
                                </div>
                            </div>
                        </div>
                            ...
                            <div className="row">
                                ...
                                <table className="table table-striped table-hover">
                                    <thead style={{borderBottom:".5px solid #ccc"}}>
                                        ...
                                        {
                                            data!=="undefined" ?(
                                            details.map((recolte,index)=>(
                                                
                                                <tr className="border border-color-2" key={"LigneTableBlockDetail"+index}>
                                                    <th
                                                    scope="row"
                                                    className="pl-6 border-0 py-7 min-width-px-235"
                                                    >
                                                    <div className="">
                                                        <Link href={"/dashboard-detail-recolte/"+recolte.id}>
                                                    ...
                                                    <td className="table-y-middle py-7 min-width-px-80">
                                                        <a
                                                            href={"/dashboard-detail-recolte/"+recolte.id}
